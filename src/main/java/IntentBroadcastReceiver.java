...
public class IntentBroadcastReceiver extends BroadcastReceiver {
	...
	public int getStringIndexfromName(String name){
		int count_resultname = ResultNameVector.size();
		for (int i = 0; i < count_resultname; i++) {
			if(ResultNameVector.get(i)!=null&&ResultNameVector.get(i).equals(name)){
				return i;
			}
		}
		return -1;
	...
	public int getArrayIndexfromName(String name){
		int count_resultname = ResultNameVector.size();
    	int count_resultarrayname = ResultArrayNameVector.size();
    	for (int i = 0; i < count_resultarrayname; i++) {
    		//Log.i("IBRcv.getArrayIndexfromName()","ResultArrayNameVector["+i+"]:"+ResultArrayNameVector.get(i));
			if(((String)ResultArrayNameVector.get(i)).equals(name)){
				//Log.i("IBRcv.getArrayIndexfromName()",""+(count_resultname+i));
				return count_resultname+i;
			}
		}
    	return -1;
	}
